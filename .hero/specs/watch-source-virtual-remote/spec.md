...
**Status:** completed — all children delivered. Live Cable DVR channel-tune still later.

**Pick up at:** when the Xfinity box is on, Watch → Cable DVR and punch a channel.

→ done

**Files:** `Panel.qml`, `Service.qml`, `.hero/planning/features/virtual-remote-numbers/spec.md`
...
|---|---|---|---|---|---|---|
| 1 | `remote-command-metadata` | completed | small | next | — | Parse `commands[]` + `navigator_display_option` into a button-capability model; no UI. |
| 2 | `virtual-remote-dpad` | completed | medium | next | `remote-command-metadata` | D-pad, Menu, Enter for the selected watch source, Halo-styled. |
| 2b | `watch-receiver-audio-options` | completed | medium | now | `virtual-remote-dpad` | Surround/audio rows for sources that declare `surround_modes`, not nav. |
| 3 | `virtual-remote-transport` | completed | small | now | `virtual-remote-dpad` | Transport cluster, shown only where the device declares it. |
| 3b | `watch-remote-lost-on-reopen` | completed | small | now | `virtual-remote-dpad` | Restore the Watch remote from `CURRENT_VIDEO_DEVICE`; do not re-SELECT. |
...
2b. **Receiver audio options** (`watch-receiver-audio-options`) — AVR Watch sources on this Director publish `capabilities.surround_modes` and **no** `commands[]`. Opening a remote only on D-pad keys leaves them as tap-to-select. Surround rows are capability-gated; extras `GET /items/{id}/commands` is out of that bug.
3. **Transport** (`virtual-remote-transport`) — gated on `commands[]`, not `show_transport`. Designed: two glyph rows under the D-pad, reuse `sendRemote`.
3b. **Restore on reopen** (`watch-remote-lost-on-reopen`) — panel open / room change reads `CURRENT_VIDEO_DEVICE` and opens the remote without `SELECT_VIDEO_DEVICE`.
...
- IF a source declares no navigation commands THE SYSTEM SHALL keep tap-to-select with no empty remote, unless it declares discrete surround modes (`watch-receiver-audio-options`)
